...
7.  ~~参数管理：对系统动态配置常用参数。config~~
8.  ~~通知公告：系统通知公告信息发布维护。notice~~
9.  ~~操作日志：系统正常操作日志记录和查询；系统异常信息日志记录和查询。operlog~~
10. ~~登录日志：系统登录日志记录查询包含登录异常。logininfor~~
11. ~~在线用户：当前系统中活跃用户状态监控。online~~
12. ~~定时任务：在线（添加、修改、删除)任务调度包含执行结果日志。job~~
13. ~~代码生成：前后端代码的生成（java、html、xml、sql）支持CRUD下载 。gen~~
14. ~~系统接口：根据业务代码自动生成相关的api接口文档。swagger~~
15. ~~服务监控：监视当前系统CPU、内存、磁盘、堆栈等相关信息。server~~
16. ~~缓存监控：对系统的缓存信息查询，命令统计等。cache~~
17. ~~在线构建器：拖动表单元素生成相应的HTML代码。build~~
18. ~~连接池监视：监视当前系统数据库连接池状态，可进行分析SQL找出系统性能瓶颈。druid~~
